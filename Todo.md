...
- [x] 로그인 화면 역할 카드 버스기사/학부모 2개로 축소 (DRIVER 카드 제거, admin 라벨 → 버스기사)
- [x] 대시보드 FAB (+) 버튼 재등장 수정 — QuickPaymentDialog 미오픈 시 null 반환으로 완전 제거
- [x] QuickPaymentDialog 오픈 시 body overflow-hidden 적용 — 뒷 배경 스크롤 방지
- [x] 입금 기록 폼 텍스트 색상 통일 — text-slate-700 → text-sp-text, 설명 텍스트 → text-sp-muted (record-payment-form, record-payment-modal, payment-quick-modal)
